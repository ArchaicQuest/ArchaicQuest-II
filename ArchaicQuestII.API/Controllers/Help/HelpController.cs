    ...
    public class HelpController : Controller
    {
        ...
        public IActionResult Post([FromBody] Help help)
        {
            ...
            string json = JsonConvert.SerializeObject(
                new { toast = "Help File created successfully", id = data.Id }
            );
        ...
        {
            return _db.GetList<Help>(DataBase.Collections.Help)
            ...
            if (saved)
            {
                return Ok(
                    JsonConvert.SerializeObject(
                        new { toast = $"{help.Title} deleted successfully." }
                    )
                );
            }
            return Ok(
                JsonConvert.SerializeObject(new { toast = $"{help.Title} deletion failed." })
            );
        }
    }
}
