...
{
    public ScanCmd(ICoreHandler coreHandler)
    {
        Aliases = new[] {"scan"};
    ...
    private void ScanDirection(Player player, Room room, string direction)
    {
        ...
        {
            Handler.Client.WriteLine("You can't look in that direction.", player.ConnectionId);
            return;
        }

        var getRoomCoords = Helpers.IsExit(getDirection, room);

        var getRoomObj = Handler.World.GetRoom($"{getRoomCoords.AreaId}{getRoomCoords.Coords.X}{getRoomCoords.Coords.Y}{getRoomCoords.Coords.Z}");
        var sb = new StringBuilder();
