...
{
    public AreaCmd(ICoreHandler coreHandler)
    {
        Aliases = new[] {"area"};
    ...
    public void Execute(Player player, Room room, string[] input)
    {
        ...
        var area = Handler.World.GetRoomArea(room);
        var roomCount = Handler.World.GetAllRoomsInArea(room.AreaId).Count;
        ...
        if (target == "list")
        {
            var areas = Handler.World.GetAllAreas();
        ...
        {
            Handler.Client.WriteLine(AreaConsider(player, room), player.ConnectionId);
            return;
        }
            
        if (target is "pop" or "population")
        {
            Handler.Client.WriteLine(AreaPopulation(room), player.ConnectionId);
            return;
        }
    ...
    private string AreaConsider(Player player, Room room)
    {
        ...
        foreach (var mob in Handler.World.GetAllRoomsInArea(room.AreaId).SelectMany(r => r.Mobs))
        {
            mobLevels += mob.Level;
            mobCount++;
        }
    ...
    {
        var playerCount = Handler.World.GetAllRoomsInArea(room.AreaId).SelectMany(r => r.Players).Count();
