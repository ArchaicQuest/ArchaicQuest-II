...
public class ListShopInventoryCmd : ICommand
{
    ...
    private void DisplayInventory(Player mob, Player player)
    {
        ...
        sb.Append(
            "<table class='data'><tr><td style='width: 30px; text-align: center;'>#</td><td style='width: 30px; text-align: center;'>Level</td><td  style='width: 65px;'>Price</td><td>Item</td></tr>"
        );
        ...
        foreach (var item in mob.Inventory.Distinct().OrderBy(x => x.Level).ThenBy(x => x.Value))
        {
            i++;
            sb.Append(
                $"<tr><td style='width: 30px; text-align: center;'>{i}</td><td style='width: 30px; text-align: center;'>{item.Level}</td><td  style='width: 65px;'>{DisplayUnit(item.Value, hagglePriceReduction)}</td><td>{item.Name}</td></tr>"
            );
        }
