...
{
    public InspectShopItemCmd()
    {
        Aliases = new[] { "inspect", "inpect", "insp", };
        Description =
            "Inspect an item that is being sold at a shop, the mob will explain the properties of the item. You may also be interested in the help files "
            + "list, buy or sell.";
    ...
    public void Execute(Player player, Room room, string[] input)
    {
        ...
        {
            CoreHandler.Instance.Writer.WriteLine("Inspect what?");
            return;
        }
        ...
        sb.Append(
            $"<p>{vendor.Name} explains to you the properties of {hasItem.Name.ToLower()}:</p>"
        );
        sb.Append(
        ...
        {
            sb.Append(
                $"Armour Type: {hasItem.ArmourType}, Defense {hasItem.ArmourRating.Armour} and {hasItem.ArmourRating.Magic} vs magic.<br />"
            );
        }
        ...
        {
            sb.Append(
                $"Weapon Type: {hasItem.WeaponType}, Damage is {hasItem.Damage.Minimum}-{hasItem.Damage.Maximum} (average {hasItem.Damage.Minimum + hasItem.Damage.Maximum / 2}).<br />"
            );
