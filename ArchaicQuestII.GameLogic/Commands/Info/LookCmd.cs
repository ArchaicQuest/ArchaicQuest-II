...
{
    public LookCmd(ICoreHandler coreHandler)
    {
        Aliases = new[] {"look", "l"};
        ...
        if (string.IsNullOrEmpty(target))
        {
            switch (verb)
            {
                case "at" when string.IsNullOrEmpty(target):
                    Handler.Client.WriteLine("<p>Look at what?</p>", player.ConnectionId);
                    return;
                case "in" when string.IsNullOrEmpty(target):
                    Handler.Client.WriteLine("<p>Look in what?</p>", player.ConnectionId);
                    return;
            }
        }
    ...
    private void Look(Player player, Room room)
    {
        ...
        var exits = Handler.World.FindValidExits(room, showVerboseExits);
        ...
        Handler.Client.WriteLine(roomDesc.ToString(), player.ConnectionId);
    }
    ...
    private void LookInContainer(Player player, Room room, string target)
    {
            ...
            container.ItemType != Item.Item.ItemTypes.Cooking)
        {
            ...
            Handler.Client.WriteLine($"<p>{container.Name} is not a container", player.ConnectionId);
            return;
        }
        ...
        {
            Handler.Client.WriteLine("<p>You need to open it first.", player.ConnectionId);
            return;
        }

        Handler.Client.WriteLine($"<p>{container.Name} contains:</p>", player.ConnectionId);
            
        if (container.Container.Items.Count == 0)
        {
            Handler.Client.WriteLine("<p>Nothing.</p>", player.ConnectionId);
        }
        ...
        {
            Handler.Client.WriteLine($"<span class='item {(isDark ? "room-dark" : "")}'>{obj.Name}</span>",
                player.ConnectionId);
        }
            
        foreach (var pc in room.Players.Where(pc => pc.Name != player.Name))
        {
            Handler.Client.WriteLine($"<p>{player.Name} looks inside {container.Name.ToLower()}.</p>",
                pc.ConnectionId);
        }
    }
    ...
    {
        var getPortalLocation = Handler.World.GetRoom(target.Portal.Destination);

        if (getPortalLocation == null)
        {
            Helpers.LogError(Handler.Db,"LookCmd.cs", $"Portal ({target.Portal.Name}) location empty.", ErrorPriority.Medium);
            Handler.Client.WriteLine("<p>The dark abyss, I wouldn't enter if I were you.</p>", player.ConnectionId);
            return;
        }
    ...
    private void LookAtObject(Player player, Room room, string target)
    {
        ...
        if (item != null)
        {
            Handler.Client.WriteLine($"<p  class='{(isDark ? "room-dark" : "")}'>{item.Description.Look}",
                player.ConnectionId);
            ...
            if (hasLore != null)
            {
                ...
                if (success)
                {
                    Handler.Character.PassiveSkills.Lore(player, room, item.Name);
                }
                else
                {
                    Handler.Character.GainSkillExperience(player, hasLore.Level * 100, hasLore, DiceBag.Roll(1, 1, 5));
                }
            }

            if (item.Container is { CanOpen: false } && item.Container.Items.Any())
            {
                Handler.Client.WriteLine($"<p  class='{(isDark ? "room-dark" : "")}'>{item.Name} contains:",
                    player.ConnectionId);
                ...
                Handler.Client.WriteLine(listOfContainerItems.ToString(), player.ConnectionId);

            }

            foreach (var pc in room.Players.Where(pc => pc.Name != player.Name))
            {
                Handler.Client.WriteLine($"<p>{player.Name} looks at {item.Name.ToLower()}.</p>", pc.ConnectionId);
            }
        ...
        if (roomObjects != null)
        {
            Handler.Client.WriteLine($"<p class='{(isDark ? "room-dark" : "")}'>{roomObjects.Look}",
                player.ConnectionId);

            foreach (var pc in room.Players.Where(pc => pc.Name != player.Name))
            {
                Handler.Client.WriteLine($"<p>{player.Name} looks at {roomObjects.Name.ToLower()}.</p>",
                    pc.ConnectionId);
            }
        ...
        if (character == null)
        {
            Handler.Client.WriteLine("<p>You don't see them here.", player.ConnectionId);
            return;
        }
        ...
        Handler.Client.WriteLine(
            $"{sb}<p class='{(isDark ? "room-dark" : "")}'>{character.Description} <br/>{character.Name} {Formulas.TargetHealth(player, character)} and is {statusText}<br/> {displayEquipment}",
            player.ConnectionId);
            
        Handler.Client.WriteToOthersInRoom($"<p>{player.Name} looks at {character.Name.ToLower()}.</p>", room, player);
    }
    ...
    private string DisplayPlayers(Room room, Player player)
    {
        ...
        foreach (var pc in room.Players.Where(pc => pc.Name != player.Name))
        {
            ...
            pcName += pc.Pose;
            players += $"<p class='player {(Handler.World.RoomIsDark(player, room) ? "dark-room" : "")}'>{pcName}.</p>";
        }
