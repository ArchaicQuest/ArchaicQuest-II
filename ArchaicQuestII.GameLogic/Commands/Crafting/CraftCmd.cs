...
{
    public CraftCmd(ICoreHandler handler)
    {
        Aliases = new[] {"craft"};
    ...
    private void CraftItem(Player player, Room room, string item)
    {
        ...
        {
            Handler.Client.WriteLine("<p>To begin crafting you require the correct tools such as a crafting bench.</p>", player.ConnectionId);
            return;
        }
        ...
        if (recipe == null)
        {
            Handler.Client.WriteLine("<p>You can't craft that.</p>", player.ConnectionId);
            return;
        }
            
        Handler.Client.WriteLine($"<p>You begin crafting {Helpers.AddArticle(recipe.Title).ToLower()}.</p>", player.ConnectionId);
        ...
        if (success)
        {
            ...
            foreach (var material in recipe.CraftingMaterials)
            {
                ...
                if (craftItem == null)
                {
                    Handler.Client.WriteLine("<p>You appear to be missing required items.</p>", player.ConnectionId);
                    return;
                }
            ...
            if (recipe.CreatedItemDropsInRoom)
            {
                ...
                Handler.Client.WriteLine($"<p>You continue working on {Helpers.AddArticle(recipe.Title).ToLower()}.</p>",
                    player.ConnectionId, 2000);

                Handler.Client.WriteLine($"<p class='improve'>You have successfully created {Helpers.AddArticle(recipe.Title).ToLower()}.</p>",
            ...
            else
            {
                ...
                Handler.Client.WriteLine("<p>You slave over the crafting bench working away.</p>",
                    player.ConnectionId, 2000);
                Handler.Client.WriteLine($"<p class='improve'>You have crafted successfully {Helpers.AddArticle(recipe.Title).ToLower()}.</p>",
                    player.ConnectionId, 4000);
            }
        ...
        else
        {
            ...
            if (recipe.CreatedItemDropsInRoom)
            {
                Handler.Client.WriteLine($"<p>You have failed in making {recipe.Title}.</p>",
                    player.ConnectionId, 2000);
            }
            else
            {
                Handler.Client.WriteLine($"<p>You have failed to craft {recipe.Title}. It looks nothing like!</p>",
                    player.ConnectionId, 2000);
            }

            Handler.Character.SkillLearnMistakes(player, "Crafting", 2000);
        }
    }
    ...
    private void List(Player player, Room room)
    {
        ...
        {
            Handler.Client.WriteLine("<p>You don't have any materials to craft a thing.</p>", player.ConnectionId);
            return;
        }

        // Lets find what you can craft
        var craftingRecipes = Handler.Item.GetCraftingRecipes();

        if (craftingRecipes == null)
        {
            Handler.Client.WriteLine("<p>No crafting recipes have been set up.</p>", player.ConnectionId);
            return;
        }
        ...
        {
            Handler.Client.WriteLine("<p>No crafting recipes found with the current materials you have.</p>", player.ConnectionId);
            return;
        }
