...
namespace ArchaicQuestII.GameLogic.Commands.Character
{
    ...
    {
        public RecallCmd(ICoreHandler coreHandler)
        {
            Aliases = new[] {"recall"};
        ...
        {
            var recallRoom = Handler.World.GetRoom(player.RecallId);
        ...
        private async void Recall(Player player, Room currentRoom, Room recallRoom)
        {
            ...
            Handler.Client.WriteLine($"<p>{_castBegin[cb]}</p>", player.ConnectionId);

            await Task.Delay(2000);
            
            Handler.Client.WriteLine($"<p>{_castEnd[ce]}</p>", player.ConnectionId);
            
            await Task.Delay(2000);
            
            Handler.World.RoomChange(player, currentRoom, recallRoom);
        }
    }
}