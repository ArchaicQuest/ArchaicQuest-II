...
namespace ArchaicQuestII.GameLogic.Commands.Character
{
    ...
    {
        public TrainCmd(ICoreHandler coreHandler)
        {
            Aliases = new[] {"train"};
        ...
        public void Execute(Player player, Room room, string[] input)
        {
            ...
            if (player.Trains <= 0)
            {
                Handler.Client.WriteLine("<p>You have no training sessions left.</p>", player.ConnectionId);
                return;
            }
            ...
            else
            {
                ...
                {
                    var hitDie = Handler.Character.GetClass(player.ClassName);
