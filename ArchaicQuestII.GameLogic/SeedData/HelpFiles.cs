    ...
    internal static class HelpFiles
    {
        private static readonly List<Help> seedData = new List<Help>()
        {
            ...
            new Help()
            {
                ...
                Description =
                    "Typing <span class='hint'>look</span> or <span class='hint'>l</span> will display the current room you are in. Look can take these parameters: <ul><li>look &lt;thing&gt;</li><li>look in &lt;container&gt;</li></ul> To look at objects in the room such as players, mobs, items or objects mentioned in the description type look &lt;thing&gt; for example: look sword.\r\n\r\nTo look inside a container type look in  &lt;container&gt; example: look in bag.\r\n\r\nFor a detailed look at objects with more verbose descriptions see <span class='hint'>Help Examine</span>\r\n\r\n",
                DateCreated = DateTime.Now,
                DateUpdated = DateTime.Now,
            ...
            new Help()
            {
                ...
                Description =
                    "Typing <span class='hint'>examine &lt;thing&gt;</span> or <span class='hint'>exam &lt;thing&gt;</span> will display detailed descriptions if they exist on a object. If none exists it defaults to the basic look description.\r\n\r\n Usage: <span class='hint'>examine sword</span>",
                DateCreated = DateTime.Now,
                DateUpdated = DateTime.Now,
        ...
        {
            if (!Services.Instance.DataBase.DoesCollectionExist(DataBase.Collections.Help))
            {
                foreach (var helpSeed in seedData)
                {
                    Services.Instance.DataBase.Save(helpSeed, DataBase.Collections.Help);
                }
            }
            else
            {
                var helpList = Services.Instance.DataBase.GetList<Help>(DataBase.Collections.Help);
            ...
            var helpFiles = Services.Instance.DataBase.GetList<Help>(DataBase.Collections.Help);
            foreach (var helpFile in helpFiles)
            {
                Services.Instance.Cache.AddHelp(helpFile.Id, helpFile);
            }
        }
    }
}
