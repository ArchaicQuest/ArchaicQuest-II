      ...
      private static readonly  Area initialArea = new Area()
        ...
        private static readonly List<Room> seedData = new List<Room>()
        {
           new Room()
           {
               ...
               Description = "You're in a dark dingy room lit only by the flicker of torch light. " +
               ...
               RoomObjects = new List<RoomObject>()
               {
                   ...
                   {
                       Name = "wooden table",
                       Look = "A dilapidated wooden table with wonky legs sits crooked in the room. There is a note upon it that you should read.",
                   }
               },
               ...
               Items = new ItemList()
               {
                   new Item.Item()
                   {
                       ...
                       Description = new Description()
                       {
                           ...
                           Look = "Thanks for trying out ArchaicQuest." +
                           ...
                           Touch = "The note feels slightly wet, probably caused by the occasional drip from above",
        ...
        {
            if (!db.DoesCollectionExist(DataBase.Collections.Area))
            { 
                    db.Save(initialArea, DataBase.Collections.Area);
            }
            
            if (!db.DoesCollectionExist(DataBase.Collections.Room))
            {
                foreach (var roomSeed in seedData)
                {
                    db.Save(roomSeed, DataBase.Collections.Room);
                }
            }
        }
    }
