...
public class ClientHandler : IClientHandler
{
    ...
    public ClientHandler(IHubContext<GameHub> hubContext, TelnetHub telnetHub)
    {
    ...
    public async void WriteLine(string message, string id)
        {
            try
            {
                if (id.Equals("mob", StringComparison.CurrentCultureIgnoreCase)) return;
    ...
    public async void WriteLineMobSay(string mobName, string message, string id)
        ...
        public void WriteLineRoom(string message, Room room)
        {
    ...
    public async void UpdateScore(Player player)
        {
            if (string.IsNullOrEmpty(player.ConnectionId) && !player.IsTelnet) return;
        ...
        public async void UpdateTime(Player player, Time time)
        {
            if (string.IsNullOrEmpty(player.ConnectionId) && !player.IsTelnet) return;

            try
            {
                await _hubContext.Clients.Client(player.ConnectionId).SendAsync("UpdateTime", time.ReturnTime());
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
